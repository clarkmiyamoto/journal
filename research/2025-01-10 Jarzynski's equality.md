...
Say you're trying to compute $$\mathbb E_{x \sim p}[f(x)]$$

where you have oracle access to $p$. The typical example is to use important sampling. What if $p$ is highly multimodal? Then you should use annealed important sampling. That is a discrete time algorithm, so I'm wondering if there is a continuous time formulation! (Similar to DDPM and SBD).

Say you have a family of distributions $(p_t)_{t \in [0,1]}$ where $$p_t(x) = e^{-V_t(x)} / Z$$

You might naively say let's use Langevin to transport the samples 
